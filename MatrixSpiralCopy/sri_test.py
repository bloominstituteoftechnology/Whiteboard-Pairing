...
from model_solution.model_solution import spiralCopy as model_spiral_copy
...
class IterativeSortingTest(unittest.TestCase):
    ...
    def test_spiralCopy(self):
        ...
        self.assertEqual(model_spiral_copy(arr1),
                         [1, 2, 3, 4, 5, 10, 15, 20, 19, 18,
                          17, 16, 11, 6, 7, 8, 9, 14, 13, 12])                          
        ...
        random.sample(range(200), 50),
        random.sample(range(200), 50)]                                     
        res2 = model_spiral_copy(arr2) 
        log.info(f'{15 * 50}: my {len(spiralCopy(arr2))} model  {len(model_spiral_copy(arr2))}')
